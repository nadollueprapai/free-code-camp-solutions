...
const updateResultsList = () => {
    ...
    units.forEach(
        (unitToConvertTo) => {
            if (unitToConvertTo != unitToConvert.value) {
                const convertedQuantity = processIngredient(ingredientQuantity.value, unitToConvert.value, unitToConvertTo, Number(numberOfServings.value));

                const listItemElement = document.createElement("li");
                listItemElement.textContent = `${ingredientName.value}: ${convertedQuantity} ${unitToConvertTo}`;
